...
var asyncFunctionPattern = '$TvNd3G0XQ1gN';
...
function isAsyncFunction(value, asyncPattern) {
    if (asyncPattern === void 0) { asyncPattern = ''; }
    if (!value)
        return false;
    return testConstructor('AsyncFunction', value) ||
        ((testConstructor('Function', value) && (value.toString().indexOf(asyncPattern.length > 1 ? asyncPattern : asyncFunctionPattern) > 0 ||
            (value.toString().indexOf('return __awaiter(this') > 0 &&
                value.toString().indexOf('return __generator(this') > 0))));
}
...
/** Is value symbol */
function isSymbol(value, name) {
    if (name === void 0) { name = false; }
    return typeof value === 'symbol' && (!name || value.description === name);
}
